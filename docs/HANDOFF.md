...
- Добавлена таблица `league_rank_aliases`: через неё можно задать `display_name` и `logo_url` для ранга. Frontend показывает логотип ранга рядом с именем игрока в верхней плашке, профиле и рейтинге.
...
- Вкладка `Лига` теперь показывает ежедневную награду, прогресс с отметками `0 ✦` и `200 000 ✦`, описание расчёта прогресса и более понятные карточки наград.
...
- SQL для существующей Supabase остаётся в `supabase/migrations/20260508_league_and_delete_cleanup.sql`, он дополнен `league_rank_aliases` и `daily_login_rewards`.
