...
## Обновление 2026-05-08: шкала уровней

- Верхняя карточка вкладки `Лига` теперь подписана как `Уровень` и показывает аватар уровня рядом с названием.
- Шкала прогресса строится по полному списку `tiers`, поэтому первым отображается `Железо` с порогом `0 ✦`.
- При раскрытии уровня промежуточные задания визуально появляются над карточкой выбранного уровня, а не под ней; подпись блока теперь `Задания до уровня ...`.
- Cache-busting query для `static/styles.css` и `static/app.js` обновлён до `20260508-level-timeline`.
