...
## Обновление 2026-05-08: рейтинг и кэфы в ставках

- В рейтинге аватар уровня пользователя перенесён справа от имени.
- В карточках событий внутри `Мои пари` и `История` добавлен коэффициент конкретного исхода, помимо общего коэффициента ставки.
- Cache-busting query для `static/styles.css` и `static/app.js` обновлён до `20260508-rating-bet-odds`.
